...
df=pd.read_csv("reddit_thread_oct.csv") # input your own csv file here. here there should be thread ID, title, thread initializer usernames, initilization date of the thread
print("thread File read") 

print(df.columns.tolist())

tusernames=df['author_name']
tdates=df['date']
ttids=df['id']
ttitles=df['title']
...
map_username_id, map_id_username = create_map_id_entity(tusernames)
map_thread_id, map_id_thread = create_map_id_entity(ttids)
...
def construct_tensor_matrix(ptids,pusernames,pdates,map_indexed_user,map_indexed_thread,map_indexed_time):
    ...
    for i in range(0,len(ptids)):
        ...
        if type(pusernames[i]) == str:
            ...
            th=ptids[i]
            Tindex = pdates[i].find("T")
            t = pdates[i][0:Tindex]
    
           
            if u in map_indexed_user and th in map_indexed_thread and t in map_indexed_time:
                uind=map_indexed_user[u]
                thind=map_indexed_thread[th]
                tind=map_indexed_time[t]
...
mat = construct_tensor_matrix(ptids,pusernames,pdates,map_username_id,map_thread_id,map_date_id)
...
def find_anomalous_component(userdataset,threaddataset,xlabel,ylabel,thresholduser,thresholdthread,part):
    ...
    for key in sorted(map_user_thread_count_component):
        ...
        print("Valid ",ylabel," count: ",map_user_thread_count_component[key][0], " Component: ",map_user_thread_count_component[key][1])
...
def returntopk(map_component_value,map_component_entitylist):
    for component in map_component_value.keys():
        print("Component ",component)
        vlaues = map_component_value[component]
        map_userid_value={}
...
print("Top k users: ")
returntopk(map_component_user_value,map_component_userlist)

print("Top k threads: ")
returntopk(map_component_thread_value,map_component_threadlist)

print("Top k dates: ")
returntopk(map_component_time_value,map_component_timelist)
